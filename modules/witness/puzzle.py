...
class Puzzle:
	def __init__(self, screen):
		...
		self.entry = Point(rdm.choice([0, self.width - 1]), rdm.choice([0, self.height - 1]))

		while True:			
			self.exit = Point(rdm.choice([0, self.width - 1]), rdm.choice([0, self.height - 1]))
	...
	def create_features(self):
		...
		total_features = rdm.randint(2, 8)
		if self.solution.zone_count == 1:
			total_features += 5

		for i in range(total_features):
			feature_type = rdm.randint(0, 2)

			if feature_type == 0 and len(available_mandatory_nodes) > 0:
				mandatory_node = rdm.choice(available_mandatory_nodes)
			...
			if feature_type == 1:
				...
				while True:
					node1 = Point(rdm.randint(0, self.width - 2), rdm.randint(0, self.height - 2))
					if bool(rdm.getrandbits(1)):
			...
			if feature_type == 2 and len(available_mandatory_edges) > 1:
				mandatory_edge = rdm.choice(available_mandatory_edges)
		...
		if self.solution.zone_count > 1:
			...
			for i in range(rdm.randint(0, 8)):
				p = Point(rdm.randint(0, self.width - 2), rdm.randint(0, self.height - 2))
				colorblocks.colors[p.x][p.y] = Color(0, 0, 0) if zones[p.x][p.y] % 2 == 0 else Color(255, 255, 255)
